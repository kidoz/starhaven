...
TEST_CASE("vec3 dot/cross/normalize", "[math3d]") {
    ...
    REQUIRE(c.x == 0); REQUIRE(c.y == 0); REQUIRE(c.z == 1);
...
TEST_CASE("mat4 * vec4 identity and translate", "[math3d]") {
    ...
    REQUIRE(v.x == 1); REQUIRE(v.y == 2); REQUIRE(v.z == 3); REQUIRE(v.w == 1);
    Vec4 t = mat4_translate({10, 20, 30}) * Vec4{1, 2, 3, 1};
    REQUIRE(t.x == 11); REQUIRE(t.y == 22); REQUIRE(t.z == 33);
}
...
TEST_CASE("camera_forward/right at yaw 0", "[math3d]") {
    ...
    REQUIRE(f.x == 0); REQUIRE(f.y == 0); REQUIRE(f.z == Approx(-1).margin(kEps));
    Vec3 r = camera_right(0);
    REQUIRE(r.x == Approx(1).margin(kEps)); REQUIRE(r.z == 0);
}
...
TEST_CASE("matrix multiply is associative with identity", "[math3d]") {
    ...
    REQUIRE(v.x == 1); REQUIRE(v.y == 2); REQUIRE(v.z == 3);
}
