...
TEST_CASE("the teachers' experience bonuses parse from words and digits", "[hire]") {
    ...
    REQUIRE(parse_benefit("Ten percent bonus on all experience learned.").experience_percent == 10);
    REQUIRE(parse_benefit("Fifteen percent bonus on all experience learned.").experience_percent ==
            15);
}
...
TEST_CASE("the guides shave their days from the right kind of travel", "[hire]") {
    REQUIRE(
        parse_benefit("All map crossings one day faster (minimum one day).").coach_days_faster ==
        1);
    REQUIRE(
        parse_benefit("All map crossings three days faster (minimum one day).").coach_days_faster ==
        3);
...
TEST_CASE("a benefit this engine has no mechanic for parses to nothing", "[hire]") {
    REQUIRE_FALSE(
        parse_benefit("Grants a constant, single category bonus to your reputation.").any());
...
TEST_CASE("the trap trades' bonuses parse", "[hire]") {
    REQUIRE(
        parse_benefit("Four point bonus to Disarm Traps skill for all characters.").disarm_bonus ==
        4);
    REQUIRE(parse_benefit("Disarm Traps skill is increased by eight points for each character "
                ...
                .disarm_bonus == 8);
    REQUIRE(
        parse_benefit("Six point bonus to perception skill for all characters.").perception_bonus ==
        6);
    REQUIRE(parse_benefit("Perception skill is increased by five points and Luck statistics "
                          "are increased by ten points for each character.")
                .perception_bonus == 5);
...
TEST_CASE("the wind and gate masters' daily casts parse", "[hire]") {
    ...
    REQUIRE(
        parse_benefit("Casts the Town Portal spell at master ranking once per day.").town_portal);
