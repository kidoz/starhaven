...
std::vector<std::uint8_t> make_body(const std::vector<Row>& rows) {
    ...
    for (int i = 0; i < 4; ++i) v.push_back(static_cast<std::uint8_t>(n >> (8 * i)));
...
std::vector<std::byte> make_entry(const std::vector<std::uint8_t>& body) {
    ...
    REQUIRE(compress(z.data(), &cap, body.data(),
    ...
    for (std::uint8_t b : z) out.push_back(static_cast<std::byte>(b));
    return out;
}
...
TEST_CASE("parses a well-formed table", "[tile_table]") {
    const auto e = entry_of({{"dirttyl", 4, 0, 0}, {"grastyl", 0, 1, 0},
                             {"grdrtNE", 0, 12, 512}});
    TileTable t;
...
TEST_CASE("a tilemap byte indexes the table directly", "[tile_table]") {
    ...
    std::vector<Row> rows;
    for (int i = 0; i < 120; ++i) rows.push_back({"filler", 0, 0, 0});
...
TEST_CASE("an index past the end resolves to nullptr, not garbage",
          "[tile_table]") {
...
TEST_CASE("a name filling all 16 bytes does not run past its field",
          "[tile_table]") {
...
TEST_CASE("rejects a count that disagrees with the payload length",
          "[tile_table]") {
