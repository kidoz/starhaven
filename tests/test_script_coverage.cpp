...
TEST_CASE("audit counts every record while deduplicating scripts and scoped events", "[coverage]") {
    ...
    REQUIRE(coverage.opcodes.at(42).scripts.size() == 2);
    REQUIRE(coverage.opcodes.at(42).events.size() == 2);
    REQUIRE(coverage.opcodes.at(42).argument_sizes.at(4) == 4);
    ...
    REQUIRE(first.str().find("LOCATION\tA.EvT\t8\t1\t2\t42\t4\tunsupported\tdoor,quest_bit\n") !=
            std::string::npos);
}
