...
void put_u32(std::vector<std::byte>& v, std::size_t off, std::uint32_t x) {
    for (int i = 0; i < 4; ++i) {
        v[off + static_cast<std::size_t>(i)] = static_cast<std::byte>((x >> (8 * i)) & 0xFF);
    }
}

// Build an archive from (name, payload, compress) triples.
std::vector<std::byte>
make_archive(const std::vector<std::tuple<std::string, std::vector<std::uint8_t>, bool>>& items) {
    ...
    for (const auto& [name, payload, compress] : items) {
        ...
        uLongf len = bound;
        REQUIRE(compress2(out.data(), &len, payload.data(), static_cast<uLong>(payload.size()),
    ...
    for (std::size_t i = 0; i < count; ++i) {
        ...
        put_u32(data, entry + kSndNameSize + 4, static_cast<std::uint32_t>(stored[i].size()));
        put_u32(data, entry + kSndNameSize + 8, static_cast<std::uint32_t>(payload.size()));
