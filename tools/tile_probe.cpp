...
void print_usage(const char* argv0) {
    std::cerr
        << "Usage: " << argv0 << " [<map.odm>] [--grep SUBSTR] [--list ARCHIVE]\n"
        << "\n"
...
constexpr std::array<const char*, 6> kArchives{
    "BITMAPS.LOD", "SPRITES.LOD", "icons.lod",
...
void for_each_entry(const Opened& a,
    ...
    if (a.is_game_lod) {
        for (const auto& e : a.game_archive.entries()) fn(e.name, e.data_size);
    } else {
        for (const auto& e : a.std_archive.entries()) fn(e.name, e.stored_size);
    }
}
...
int main(int argc, char** argv) {
    ...
    for (const char* name : kArchives) {
        ...
        std::printf("  %-14s  %10ju bytes  fnv1a=%016llx  entries=%zu%s\n", name,
                    static_cast<std::uintmax_t>(fs::file_size(p)),
                    static_cast<unsigned long long>(fnv1a(p)),
                    ok ? (o.is_game_lod ? o.game_archive.entries().size()
                                        : o.std_archive.entries().size())
                       : 0,
                    ok ? "" : "  [could not parse]");
        if (ok) opened.emplace_back(p, std::move(o));
    }
    ...
    if (!list_archive.empty()) {
        ...
        for (const auto& [path, a] : opened) {
            if (lower(a.name) != lower(list_archive)) continue;
            for_each_entry(a, [&](const std::string& n, std::uint32_t sz) {
                std::printf("  %-24s %8u\n", n.c_str(), sz);
            });
        }
    ...
    for (std::size_t i = 0; i < map.header.tilesets.size(); ++i) {
        std::printf("  tileset[%zu]  : group=%d offset=%d\n", i,
                    map.header.tilesets[i].group, map.header.tilesets[i].offset);
    }

    std::map<std::uint8_t, int> hist;
    for (std::uint8_t t : terrain.tilemap) ++hist[t];
    ...
    while (!stem.empty() && stem.back() == '\0') stem.pop_back();
    if (stem.size() > 4) stem = stem.substr(0, 4);
    if (!stem.empty()) {
        std::cout << "\n== BITMAPS.LOD entries starting with \"" << stem
                  << "\" (ground tileset candidates) ==\n";
        for (const auto& [path, a] : opened) {
            if (lower(a.name).find("bitmaps") == std::string::npos) continue;
            int hits = 0;
            for_each_entry(a, [&](const std::string& n, std::uint32_t sz) {
