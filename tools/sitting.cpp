...
void print_usage(const char* argv0) {
    std::cerr << "Usage: " << argv0 << " [<map>[,<map>...]] [--minutes N] [--seed N] [--fps N]\n"
...
int main(int argc, char** argv) {
    ...
    for (std::size_t at = 0; at <= class_list.size() && !class_list.empty();) {
        ...
        std::string one = class_list.substr(at, comma == std::string::npos ? comma : comma - at);
        if (!one.empty()) {
    ...
    for (const auto& who : party) {
        std::cout << "  " << std::setw(12) << std::left << who.name << std::setw(10)
                  ...
                  << who.armor_class << " age " << std::setw(4) << who.age << " might "
                  << who.attribute(game::Attribute::Might) << " (" << std::showpos
                  << game::attribute_bonus(who.attribute(game::Attribute::Might)) << std::noshowpos
                  << ")"
                  << "  speed " << who.attribute(game::Attribute::Speed) << " (" << std::showpos
                  << game::attribute_bonus(who.attribute(game::Attribute::Speed)) << std::noshowpos
                  << ")"
    ...
    for (const std::string& map_name : maps) {
        ...
        if (world::load_map_session(game::resolve_games_lod(), data_dir, map_name, cache,
        ...
        if (arm) {
            ...
            for (const auto& who : party) {
                ...
                for (const auto slot : {game::Slot::Weapon, game::Slot::Armor}) {
                    ...
                    const auto* row =
                        piece > 0 ? items.at(static_cast<std::size_t>(piece)) : nullptr;
        ...
        for (int step = 0; step < steps; ++step) {
            ...
            if (const std::size_t near =
                    advance && anyone_up ? nearest_alive(session, battle, eye) : game::kNoActor;
                near != game::kNoActor) {
            ...
            for (std::size_t who = 0; who < party.size(); ++who) {
                ...
                const std::size_t target = nearest_alive(session, battle, eye, game::kPartyReach);
                ...
                if (casting && party[who].spell_points > 0) {
                    ...
                    for (const int id : party[who].known_spells) {
                        ...
                        if (const int slot = game::buff_slot_of_spell(id);
                            ...
                            party_buffs.cast(slot, clock.minutes() + 60 * (points + 1), points,
                                             points);
                            raised = true;
                        } else if (const int mine = game::character_slot_of_spell(id);
                                   mine >= 0 &&
                                   party[who].buffs.power(mine, clock.minutes()) <= 0) {
                            party[who].buffs.cast(mine, clock.minutes() + 60 * (points + 1),
                                                  points);
                            raised = true;
                        }
                        if (raised) {
                ...
                if (casting && !party[who].known_spells.empty() && party[who].spell_points > 0) {
                    ...
                    for (const int id : party[who].known_spells) {
                        ...
                        const int worth = game::roll_spell_damage(
                            id, school_points(party[who], *row), [] { return 3ULL; });
                        if (worth > best) {
                            best = worth;
                            chosen = id;
                        }
                    ...
                    if (chosen > 0) {
                        ...
                        battle.smite(target, flat, scaled, points, row->element, party[who].name,
                                     session, monsters, items, random_items, standard_bonuses,
                                     special_bonuses);
                        ++tally[who].casts;
                ...
                for (const game::Slot worn : {game::Slot::Armor, game::Slot::Shield}) {
                    ...
                    points +=
                        game::worn_recovery_penalty(game::gear_recovery(row->skill_group), lift);
                }
            ...
            for (std::size_t at = 0; at < session.actors.size(); ++at) {
                ...
                const int reach =
                    game::octagonal_distance(static_cast<int>(dx), static_cast<int>(dz));
                float step = 0.0F;
            ...
            battle.award(party,
                         game::best_hired(hirelings, &game::HireBenefit::experience_percent));
                ...
                if (hall != nullptr) {
                    ...
                    if (hirelings.size() < 2) {
                        ...
                        if (cheapest != nullptr) {
                            ...
                            hirelings.push_back(
                                {game::parse_benefit(cheapest->party_benefit), cheapest->name});
                            ++hired;
                        }
                    }
                    ...
                    for (auto& who : party) {
                        for (const auto slot : {game::Slot::Weapon, game::Slot::Armor}) {
                            ...
                            const auto* have =
                                held > 0 ? items.at(static_cast<std::size_t>(held)) : nullptr;
                            int best = 0;
    ...
    const double real_minutes = static_cast<double>(steps) * static_cast<double>(maps.size()) /
                                static_cast<double>(fps) / 60.0;
    std::cout << "\nAfter " << minutes_each * static_cast<int>(maps.size()) << " world minutes ("
              << real_minutes << " real, " << steps * static_cast<int>(maps.size())
              << " steps) across " << maps.size() << " map(s) and " << actors_at_start
              << " actors\n";
    for (std::size_t who = 0; who < party.size(); ++who) {
        ...
        std::cout << ", dealt " << std::setw(5) << t.dealt << ", took " << std::setw(5) << t.taken
                  << ", low " << std::setw(4) << t.low_water << " of " << party[who].max_hit_points
                  << ", level " << party[who].level << ", xp " << party[who].experience << ", cast "
                  << t.casts << " for " << t.spell_damage;
        if (t.down_at >= 0) {
    ...
    std::cout << "  " << trained << " levels trained for at the hall; " << levels
              << " levels gained; " << killed << " actors killed; " << experience << " experience, "
              << gold << " gold\n";
    ...
    if (train_points > 0) {
        ...
        for (std::size_t who = 0; who < party.size(); ++who) {
            ...
            std::cout << (who == 0 ? "" : " ") << party[who].name.substr(0, 4) << " best " << best
    ...
    std::cout << "  the higher lines: " << stunned << " stuns, " << tripled << " triple strikes, "
              << feathered << " second arrows\n";
    return 0;
}
